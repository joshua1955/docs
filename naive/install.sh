#!/usr/bin/env bash
  ...
  check_os() {
    ...
    case "${ID:-}" in
      almalinux)
        [[ "${VERSION_ID%%.*}" == "10" ]] || warn "Скрипт рассчитан на AlmaLinux 10.
  Сейчас: ${VERSION_ID:-unknown}"
        ;;
      rocky|rhel|centos)
        warn "Скрипт рассчитан на AlmaLinux 10, но ${ID} ${VERSION_ID:-?} должен
  подойти."
        ;;
      *)
        warn "Скрипт рассчитан на AlmaLinux 10. Сейчас: ${ID:-unknown}
  ${VERSION_ID:-unknown}"
        ;;
    esac
  }
  ...
  install_packages() {
    ...
    dnf -y makecache
    dnf -y install curl wget openssl ca-certificates firewalld iproute tar policycoreutils-python-utils
  }
...
EOF
  ...
  install_go() {
    ...
    [[ "$go_version" =~ ^go[0-9]+\.[0-9]+(\.[0-9]+)?$ ]] || die "Не удалось получить
  версию Go."
  ...
  then
      log "Настройка SELinux для Caddy..."
      semanage fcontext -a -t bin_t '/usr/bin/caddy' 2>/dev/null || true
      restorecon -v /usr/bin/caddy >/dev/null || true
  ...
  <head>
    ...
    <title>Loading</title>
    <style>
      body{background:linear-gradient(135deg,#0f172a,#1e293b);height:100vh;margin:0;di
  splay:flex;flex-direction:column;align-items:center;justify-content:center;font-fami
  ly:sans-serif}
      .spinner{width:40px;height:40px;border-radius:50%;border:3px solid
  rgba(255,255,255,0.12);border-top-color:#38bdf8;animation:spin 0.8s linear
  infinite;margin-bottom:25px;box-shadow:0 0 18px rgba(56,189,248,0.25)}
